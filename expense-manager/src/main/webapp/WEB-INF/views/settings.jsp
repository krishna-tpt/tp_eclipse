...
<div class="tabs">
	...
	<button class="tab" onclick="switchTab('subcatTab', this)">Sub
		Categories</button>
		...
		<div class="card">
			...
			<form action="${pageContext.request.contextPath}/settings"
				...
				<input type="hidden" name="action" value="addCategory"> <input
					type="hidden" name="type" value="INCOME"> <input
				...
				<table>
					<tbody>
						...
						<c:if test="${empty incomeCategories}">
							<tr>
								<td class="empty-state">No income categories</td>
							</tr>
						</c:if>
					</tbody>
				</table>
				...
				<table>
					<tbody>
						...
						<c:if test="${empty expenseCategories}">
							<tr>
								<td class="empty-state">No expense categories</td>
							</tr>
						</c:if>
					</tbody>
				</table>
...
<div id="subcatTab" class="tab-panel" style="display: none">
	<div class="card">
		...
		<form action="${pageContext.request.contextPath}/settings"
			...
			<div class="form-group">
				<label>Sub Category Name *</label> <input type="text" name="name"
					placeholder="e.g. Dinner" required
					onkeydown="if(event.key==='Enter'){event.preventDefault();this.closest('form').submit();}">
			</div>
			<div class="form-group"
				style="justify-content: flex-end; padding-top: 1.4rem">
				...
				<tbody>
					<c:forEach var="sc" items="${allSubCategories}">
						<tr>
							<td>${sc.name}</td>
							<td>
								<%-- Find parent name from combined list --%> <span class="chip">
									<c:forEach var="cat" items="${incomeCategories}">
										<c:if test="${cat.id == sc.category_id}">${cat.name} (I)</c:if>
									</c:forEach> <c:forEach var="cat" items="${expenseCategories}">
					...
					<c:if test="${empty allSubCategories}">
						<tr>
							<td colspan="3" class="empty-state">No sub-categories yet</td>
						</tr>
					</c:if>
				</tbody>
			</table>
...
<div id="colTab" class="tab-panel" style="display: none">
	<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem">
		...
		<div class="card">
			...
			<form action="${pageContext.request.contextPath}/settings"
				method="post" class="flex gap-1 mb-2">
				<input type="hidden" name="action" value="addColumn"> <input
					type="hidden" name="type" value="INCOME"> <input
					type="text" name="colName" placeholder="Column name" class="full-w"
					required
					onkeydown="if(event.key==='Enter'){event.preventDefault();this.closest('form').submit();}">
				<button type="submit" class="btn btn-success btn-sm"
					style="white-space: nowrap">+ Add</button>
				...
				<table>
					<tbody>
						...
						<c:if test="${empty incomeColumns}">
							<tr>
								<td colspan="3" class="empty-state">No custom columns</td>
							</tr>
						</c:if>
					</tbody>
				</table>
