...
<div class="page-header flex">
	<div>
		...
		<p>${sessionScope.activeBookName}—last 6 months</p>
	</div>
...
<div class="chart-card mb-2"
	style="background: #fff; border: 1px solid var(--border); border-radius: var(--radius); padding: 1.25rem">
...
new Chart(document.getElementById('monthlyChart'), {
  ...
  options:{responsive:true,maintainAspectRatio:false,
    ...
    scales:{x:{grid:{display:false}},y:{ticks:{callback:v=>'₹'+Number(v).toLocaleString('en-IN')}}}}
});

if (expCat.length > 0) new Chart(document.getElementById('expCatChart'), {
  type:'doughnut',
  data:{labels:expCat.map(e=>e.name),
        datasets:[{data:expCat.map(e=>e.total),backgroundColor:PALETTE,borderWidth:2,borderColor:'#fff'}]},
  options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'right',labels:{font:{size:11}}}}}
});

if (incCat.length > 0) new Chart(document.getElementById('incCatChart'), {
  type:'doughnut',
  data:{labels:incCat.map(e=>e.name),
