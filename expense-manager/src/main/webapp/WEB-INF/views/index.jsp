...
<div class="page-header flex">
	<div>
		...
		<p>
			<c:choose>
				<c:when test="${not empty sessionScope.activeBookName}">${sessionScope.activeBookName} — financial overview</c:when>
	...
	<div class="detail-panel" id="detailPanel">
		...
		<div id="dpBody" style="padding: 1rem">
			<div
				...
				Click a transaction row to view details, edit, receipts &amp; audit
				log</div>
		</div>
	</div>
</div>
...
<div id="emailModal" class="modal-overlay">
	<div class="modal">
		...
		<form action="${pageContext.request.contextPath}/export" method="post">
			<c:if test="${not empty fqs}">
				<input type="hidden" name="filteredExport" value="1">
				<c:if test="${filter.dateFrom != null}">
					<input type="hidden" name="dateFrom" value="${filter.dateFrom}">
				</c:if>
				<c:if test="${filter.dateTo   != null}">
					<input type="hidden" name="dateTo" value="${filter.dateTo}">
				</c:if>
				<c:if test="${not empty filter.type}">
				...
				<c:if test="${not empty filter.amountOp1}">
					<input type="hidden" name="amountOp1" value="${filter.amountOp1}">
					<input type="hidden" name="amount1" value="${filter.amount1}">
				</c:if>
				<c:if test="${not empty filter.amountOp2}">
					<input type="hidden" name="amountOp2" value="${filter.amountOp2}">
					<input type="hidden" name="amount2" value="${filter.amount2}">
				</c:if>
			</c:if>
