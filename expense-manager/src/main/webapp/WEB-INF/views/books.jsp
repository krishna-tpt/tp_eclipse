...
<div class="page-header flex">
	...
	<button class="btn btn-primary ml-auto"
		onclick="openModal('newBookModal')">+ New Book</button>
</div>
...
<div
	...
	<c:forEach var="book" items="${books}">
		...
		<div class="card"
			style="border:2px solid ${isActive ? 'var(--primary)' : 'var(--border)'};position:relative;cursor:pointer"
			onclick="window.location='${isActive ? pageContext.request.contextPath.concat('/home') : pageContext.request.contextPath.concat('/books?select=').concat(book.id)}'">
			<c:if test="${isActive}">
				<span
				...
				<c:choose>
					...
					<c:otherwise>
						<a
							...
							class="btn btn-primary btn-sm">Open Book</a>
					</c:otherwise>
				</c:choose>
