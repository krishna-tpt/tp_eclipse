...
public class TransactionDAO {
    ...
    public int insert(Transaction t) throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            if (t.getSubcategoryid() > 0) ps.setInt(5, t.getSubcategoryid());
            else ps.setNull(5, Types.INTEGER);
            ps.setString(6, t.getNote());
            if (t.getBookId() > 0) ps.setInt(7, t.getBookId());
            else ps.setNull(7, Types.INTEGER);

            ResultSet rs = ps.executeQuery();
            rs.next();
            int newId = rs.getInt(1);
            if (!t.getCustomValues().isEmpty()) insertCustomValues(conn, newId, t.getCustomValues());
    ...
    public void update(Transaction oldT, Transaction newT) throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            if (newT.getSubcategoryid() > 0) ps.setInt(4, newT.getSubcategoryid());
        ...
        if (oldT.getAmount().compareTo(newT.getAmount()) != 0) {
            auditDAO.logUpdate(oldT.getId(), "user", "amount",
                    "₹" + oldT.getAmount(), "₹" + newT.getAmount());
        }
        // DateTime
        if (!oldT.getDateTime().equals(newT.getDateTime())) {
            auditDAO.logUpdate(oldT.getId(), "user", "datetime",
                    oldT.getDateTime().format(DT_FMT),
                    newT.getDateTime().format(DT_FMT));
        }
        // Category
        if (oldT.getCategoryId() != newT.getCategoryId()) {
            auditDAO.logUpdate(oldT.getId(), "user", "category",
                    nvl(oldT.getCategoryName()), nvl(newT.getCategoryName()));
        }
        // SubCategory
        if (oldT.getSubcategoryid() != newT.getSubcategoryid()) {
            auditDAO.logUpdate(oldT.getId(), "user", "subcategory",
                    nvl(oldT.getSubCategoryName()), nvl(newT.getSubCategoryName()));
        }
        // Note
        if (!Objects.equals(oldT.getNote(), newT.getNote())) {
            auditDAO.logUpdate(oldT.getId(), "user", "note",
                    nvl(oldT.getNote()), nvl(newT.getNote()));
        }
    }
    ...
    public List<Transaction> findAll(String typeFilter, int page, int pageSize,
    ...
    public int count(String typeFilter, Integer bookId) throws SQLException {
        ...
        if (bookId != null && bookId > 0) sql.append(" AND book_id = ").append(bookId);
        if (typeFilter != null && !typeFilter.isBlank())
    ...
    public List<Map<String, Object>> expenseByCategory(Integer bookId) throws SQLException {
        StringBuilder sql = new StringBuilder("""
            ...
            """);
        if (bookId != null && bookId > 0) sql.append(" AND t.book_id=").append(bookId);
    ...
    // ── Helpers ───────────────────────────────────────────
    private void insertCustomValues(Connection conn, int txnId,
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            for (Map.Entry<String, String> e : values.entrySet()) {
                ps.setInt(1, txnId); ps.setString(2, e.getValue()); ps.setString(3, e.getKey());
                ps.addBatch();
            }
            ps.executeBatch();
        }
    ...
    private void loadCustomValues(Connection conn, List<Transaction> list) throws SQLException {
        ...
        for (Transaction t : list) { ids.add(String.valueOf(t.getId())); byId.put(t.getId(), t); }
        String sql = """
        ...
        try (Statement st = conn.createStatement(); ResultSet rs = st.executeQuery(sql)) {
            while (rs.next()) {
                ...
                if (t != null) t.addCustomValue(rs.getString("col_key"), rs.getString("value"));
            }
        }
    }
