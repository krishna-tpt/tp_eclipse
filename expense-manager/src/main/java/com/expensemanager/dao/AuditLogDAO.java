...
public class AuditLogDAO {
	...
	/** Log receipt upload */
	public void logReceiptUpload(int transactionId, String changedBy, String fileName) throws SQLException {
		insert(transactionId, "RECEIPT_ADD", changedBy, "receipt", null, fileName, "Receipt uploaded: " + fileName);
	}

	/** Log receipt delete */
	public void logReceiptDelete(int transactionId, String changedBy, String fileName) throws SQLException {
	...
	private void insert(int transactionId, String action, String changedBy, String fieldName, String oldValue,
			String newValue, String note) throws SQLException {
		String sql = """
	...
	public List<AuditLog> findRecentByBook(int bookId, int page, int pageSize) throws SQLException {
		...
		try (PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			List<AuditLog> list = new ArrayList<>();
			while (rs.next()) {
				AuditLog a = mapRow(rs);
				// Extra join fields
				a.setTxnAmount(rs.getBigDecimal("amount"));
				a.setTxnCategoryName(rs.getString("cat_name"));
				a.setTxnDate(rs.getString("txn_date"));
				list.add(a);
			}
			return list;
		} finally {
	...
	public int countByBook(int bookId) throws SQLException {
		String sql = """
				SELECT COUNT(*) FROM transaction_audit_log a
				JOIN transactions t ON a.transaction_id = t.id
				WHERE t.book_id = ?
				""";
		Connection conn = db.getConnection();
...
//		a.setChangedBy(rs.getString("changed_by"));
		...
		if (ts != null)
			a.setChangedAt(ts.toLocalDateTime());
