...
public class ExportService {
    ...
    // ── PDF ──────────────────────────────────────────────────────────────
    public byte[] generatePDF(CashBook book, List<Transaction> txns,
                               BigDecimal income, BigDecimal expense) throws Exception {
        ...
        addSummaryCell(summary, "Balance",
                "₹" + income.subtract(expense),
                income.compareTo(expense) >= 0
        ...
        for (Transaction t : txns) {
            ...
            com.itextpdf.text.Font typeFont = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 8,
                    t.getType() == Transaction.Type.INCOME
                            ? new BaseColor(21, 128, 61) : new BaseColor(185, 28, 28));
            PdfPCell typeCell = new PdfPCell(new Phrase(t.getType().name(), typeFont));
            typeCell.setBackgroundColor(t.getType() == Transaction.Type.INCOME
            ...
            PdfPCell amtCell = new PdfPCell(new Phrase(
                    (t.getType() == Transaction.Type.INCOME ? "+" : "-") + "₹" + t.getAmount(), amtFont));
    ...
    // ── Excel ────────────────────────────────────────────────────────────
    public byte[] generateExcel(CashBook book, List<Transaction> txns,
                                 BigDecimal income, BigDecimal expense) throws Exception {
        ...
        try (XSSFWorkbook wb = new XSSFWorkbook()) {
            ...
            Font iFont = wb.createFont(); iFont.setColor(IndexedColors.DARK_GREEN.getIndex()); iFont.setBold(true);
            incomeStyle.setFont(iFont);

            CellStyle expStyle = wb.createCellStyle();
            Font eFont = wb.createFont(); eFont.setColor(IndexedColors.RED.getIndex()); eFont.setBold(true);
            ...
            CellStyle ts = wb.createCellStyle(); ts.setFont(tf);
