...
/**
 * GET  /transaction?id=X        → detail page with audit log
 * POST /transaction (action=update) → update transaction
 * POST /transaction (action=delete) → delete transaction
 */
...
public class EditTransactionServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            catDAO.findByType(catType).stream()
                  .filter(c -> c.getId() == updated.getCategoryId())
                  .findFirst()
            ...
            if (subcatStr != null && !subcatStr.isBlank()) {
                ...
                new SubCategoryDAO().findAll().stream()
                        .filter(sc -> sc.getId() == subcatId)
                        .findFirst()
            ...
            resp.sendRedirect(req.getContextPath() + "/transaction?id=" + id
