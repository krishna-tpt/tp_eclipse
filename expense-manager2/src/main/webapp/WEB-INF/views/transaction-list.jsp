...
<head>
    ...
    <style>
        .amt-cell { font-family:'Syne',sans-serif; font-weight:700; }
        .amt-income  { color:#10b981; }
        .amt-expense { color:#ef4444; }
        ...
        .col-mgr h4 { font-family:'Syne',sans-serif; font-size:.9rem; font-weight:700; margin-bottom:14px;
                      display:flex; align-items:center; gap:8px; }
        .col-chips  { display:flex; flex-wrap:wrap; gap:8px; margin-bottom:14px; }
        .col-chip {
            display:inline-flex; align-items:center; gap:6px;
            background:rgba(99,102,241,.1); border:1px solid rgba(99,102,241,.25);
            color:#818cf8; border-radius:20px; padding:4px 12px; font-size:.78rem;
        }
        .col-chip-del {
            background:none; border:none; cursor:pointer; color:#f87171;
            font-size:.7rem; padding:0; line-height:1;
        }
        .add-col-form { display:flex; gap:10px; flex-wrap:wrap; align-items:flex-end; }
        .add-col-form .form-group { margin-bottom:0; flex:1; min-width:140px; }
        ...
        .modal-box { background:var(--card-bg); border:1px solid var(--border);
            border-radius:18px; width:100%; max-width:420px; padding:28px;
        ...
        .empty-state i { font-size:2.8rem; opacity:.25; display:block; margin-bottom:14px; }
    </style>
</head>
<body>
...
<nav class="sidebar">
    ...
    <ul class="nav-links">
        ...
        <li><a href="${pageContext.request.contextPath}/income/list"  class="${type=='income'  ? 'active':''}"><i class="fas fa-arrow-trend-up"></i><span>Income</span></a></li>
        <li><a href="${pageContext.request.contextPath}/expense/list" class="${type=='expense' ? 'active':''}"><i class="fas fa-arrow-trend-down"></i><span>Expenses</span></a></li>
...
<main class="main-content">
    <div class="topbar">
        ...
        <div class="topbar-actions">
            <a href="${pageContext.request.contextPath}/${type}/add" class="btn ${type=='income' ? 'btn-success' : 'btn-danger'} btn-sm">
    ...
    <div class="page-body">
        ...
        <div class="col-mgr">
            <h4><i class="fas fa-table-columns" style="color:var(--accent)"></i> Custom Columns
                <span style="font-size:.75rem;font-weight:400;color:var(--text-muted)">(add extra fields to every record)</span>
            </h4>

            <div class="col-chips">
                <c:choose>
                    <c:when test="${empty customColumns}">
                        <span style="font-size:.8rem;color:var(--text-muted)">No custom columns yet.</span>
                    </c:when>
                    <c:otherwise>
                        <c:forEach var="col" items="${customColumns}">
                            <span class="col-chip">
                                ...
                                ${col.columnLabel}
                                <form method="post" action="${pageContext.request.contextPath}/${type}/list" style="margin:0">
                                    <input type="hidden" name="action"   value="deleteColumn">
                                    <input type="hidden" name="columnId" value="${col.id}">
                                    <button type="submit" class="col-chip-del" title="Remove column"
            ...
            <form method="post" action="${pageContext.request.contextPath}/${type}/list" class="add-col-form">
                <input type="hidden" name="action" value="addColumn">
                <div class="form-group">
                    <label>Column Label</label>
                    <input type="text" name="columnLabel" class="form-control" placeholder="e.g. Invoice No, Tag, Ref" required>
                </div>
                <div class="form-group">
                    <label>Type</label>
        ...
        <!-- ── Filter Bar ── -->
        <form method="get" action="${pageContext.request.contextPath}/${type}/list" class="filter-bar">
            ...
            <div class="filter-group">
                ...
                <select name="sortBy" class="form-control">
                    <option value="transaction_date" ${param.sortBy=='transaction_date'||empty param.sortBy ? 'selected':''}>Date</option>
            ...
            <div class="filter-group">
                ...
                <select name="sortDir" class="form-control">
                    <option value="DESC" ${param.sortDir=='DESC'||empty param.sortDir ? 'selected':''}>Newest First</option>
                    <option value="ASC"  ${param.sortDir=='ASC'  ? 'selected':''}>Oldest First</option>
                </select>
            </div>
            <button type="submit" class="btn btn-primary btn-sm" style="align-self:flex-end">
                <i class="fas fa-filter"></i> Filter
            </button>
            <a href="${pageContext.request.contextPath}/${type}/list" class="btn btn-secondary btn-sm" style="align-self:flex-end">
        ...
        <div class="totals-bar">
            <div class="total-chip ${type=='income' ? 'income-chip' : 'expense-chip'}">
            ...
            <c:choose>
                <c:when test="${empty transactions}">
                    <div class="empty-state">
                        ...
                        <p>No ${type} records found. <a href="${pageContext.request.contextPath}/${type}/add" style="color:var(--accent)">Add one now →</a></p>
                    </div>
                </c:when>
                <c:otherwise>
                    ...
                    <table class="data-table">
                        ...
                        <c:forEach var="t" items="${transactions}" varStatus="st">
                            <tr>
                                ...
                                <td>
                                    <span style="background:rgba(99,102,241,.1);color:#818cf8;padding:3px 9px;border-radius:6px;font-size:.78rem">
                                        ${t.category}
                                    </span>
                                </td>
                                <td class="amt-cell ${type=='income' ? 'amt-income' : 'amt-expense'}">
                                    ${type=='income' ? '+' : '-'}₹<fmt:formatNumber value="${t.amount}" pattern="#,##0.00"/>
                                </td>
                                ...
                                <td>
                                    <div class="action-btns">
                                        <a href="${pageContext.request.contextPath}/${type}/edit?id=${t.id}"
                                           class="btn-tbl btn-edit" title="Edit"><i class="fas fa-pen"></i></a>
...
<div class="modal-overlay" id="deleteModal">
    <div class="modal-box">
        <h3 style="color:#ef4444"><i class="fas fa-trash"></i> Delete Record</h3>
        <p>Are you sure you want to delete this ${type} record? This action cannot be undone.</p>
