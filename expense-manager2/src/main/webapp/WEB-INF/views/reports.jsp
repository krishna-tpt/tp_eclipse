	...
	<nav class="sidebar">
		...
		<ul class="nav-links">
			...
			<li><a href="${pageContext.request.contextPath}/reports"
				class="active"><i class="fas fa-chart-bar"></i><span>Reports</span></a></li>
	...
	<main class="main-content">
		...
		<div class="page-body">
			...
			<form method="get"
				...
				class="filter-bar" style="margin-bottom: 22px">
				...
				<div class="filter-group">
					<label>Year (trend)</label> <select name="year"
						class="form-control">
			...
			<div class="charts-grid">
				<div class="chart-card">
					<h4>
						<i class="fas fa-chart-line" style="color: #818cf8"></i> Monthly
						Trend – ${selectedYear}
					</h4>
				...
				<div class="chart-card">
					<h4>
						<i class="fas fa-chart-pie" style="color: #f59e0b"></i> Expense by
						Category
					</h4>
					<div class="chart-container">
				...
				<div class="chart-card">
					<h4>
						<i class="fas fa-chart-bar" style="color: #10b981"></i> Income by
						Category
					</h4>
					<div class="chart-container">
						<canvas id="incBarChart"></canvas>
					</div>
				</div>
				<div class="chart-card">
					<h4>
						<i class="fas fa-scale-balanced" style="color: #6366f1"></i>
						Income vs Expense
					</h4>
					<div class="chart-container">
					...
					<div style="padding: 4px 0">
						<c:choose>
							<c:when test="${empty incomeByCategory}">
								<p
									style="padding: 20px; color: var(--text-muted); font-size: .85rem">No
									data.</p>
							</c:when>
							<c:otherwise>
									...
									<tbody>
										<c:forEach var="row" items="${incomeByCategory}">
											<tr>
												...
												<td style="color: #10b981; font-weight: 600">₹<fmt:formatNumber
														value="${row.total}" pattern="#,##0.00" /></td>
												<td><c:if test="${totalIncome > 0}">
														<div class="progress-bar-wrap">
															<div class="progress-bar-fill"
																style="background:#10b981;width:${(row.total/totalIncome)*100}%"></div>
														</div>
													</c:if></td>
											</tr>
										</c:forEach>
									...
									<tbody>
										<c:forEach var="row" items="${expenseByCategory}">
											<tr>
												<td>${row.category}</td>
												<td style="color: var(--text-muted)">${row.count}</td>
												<td style="color: #ef4444; font-weight: 600">₹<fmt:formatNumber
														value="${row.total}" pattern="#,##0.00" /></td>
												<td><c:if test="${totalExpense > 0}">
														<div class="progress-bar-wrap">
															<div class="progress-bar-fill"
																style="background:#ef4444;width:${(row.total/totalExpense)*100}%"></div>
														</div>
													</c:if></td>
											</tr>
										</c:forEach>
