...
public class BackupScheduler implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent sce) {
		try {
			new com.expense.dao.BackupDAO().createTableIfNotExists();
			LOG.info("[BackupScheduler] Table ready.");
		} catch (Exception e) {
			LOG.severe("[BackupScheduler] Table create failed: " + e.getMessage());
		}
		...
		LOG.info(String.format("[BackupScheduler] Daily backup at %02d:%02d. First in %d min.", hour, minute,
				delay / 60));
	}
