...
public class rand extends FunctionCalls {
   public void toOracle(SelectQueryStatement to_sqs, SelectQueryStatement from_sqs) throws ConvertException {
      if (from_sqs == null || !from_sqs.isDenodo()) {
         this.functionName.setColumnName("");
         this.setOpenBracesForFunctionNameRequired(false);
         Vector arguments = new Vector();
         arguments.add("dbms_random.value");
         this.setFunctionArguments(arguments);
      }
   }

   public void toMSSQLServer(SelectQueryStatement to_sqs, SelectQueryStatement from_sqs) throws ConvertException {
      this.functionName.setColumnName("RAND");
      Vector arguments = new Vector();
   ...
   public void toSnowflake(SelectQueryStatement to_sqs, SelectQueryStatement from_sqs) throws ConvertException {
      this.functionName.setColumnName("RANDOM");
      Vector arguments = new Vector();
   ...
   public void toPostgreSQL(SelectQueryStatement to_sqs, SelectQueryStatement from_sqs) throws ConvertException {
      ...
      if (!arguments.isEmpty()) {
         String query = "(RANDOM() * GREATEST(1," + arguments.get(0).toString() + "))";
         if (from_sqs != null && !isPostgreLiveDbs && from_sqs.getBooleanValues("use.udf.functions.for.numeric")) {
            query = "(RANDOM() * GREATEST(1,TONUMERIC_UDF(" + arguments.get(0).toString() + ")))";
         }
   ...
   public void toTimesTen(SelectQueryStatement to_sqs, SelectQueryStatement from_sqs) throws ConvertException {
      ...
      if (this.functionArguments.size() == 0) {
         ...
         throw new ConvertException("\nThe function RAND is not supported in TimesTen 5.1.21\n");
      }
   }
   ...
   public void toVectorWise(SelectQueryStatement to_sqs, SelectQueryStatement from_sqs) throws ConvertException {
      this.functionName.setColumnName("RANDOMF");
      Vector arguments = new Vector();
      if (this.functionArguments.size() == 1) {
         arguments.addElement("0.0");
      }

      for(int i_count = 0; i_count < this.functionArguments.size(); ++i_count) {
         if (this.functionArguments.elementAt(i_count) instanceof SelectColumn) {
            arguments.addElement("((" + ((SelectColumn)this.functionArguments.elementAt(i_count)).toVectorWiseSelect(to_sqs, from_sqs).toString() + ") * 1.0)");
         } else {
            arguments.addElement("((" + this.functionArguments.elementAt(i_count).toString() + ") *1.0)");
         }
      }
   ...
   public void toSqlite(SelectQueryStatement to_sqs, SelectQueryStatement from_sqs) throws ConvertException {
      ...
      String qry = "abs(random()/pow(10,19))";
   ...
   public void toExcel(SelectQueryStatement to_sqs, SelectQueryStatement from_sqs) throws ConvertException {
      this.functionName.setColumnName("Rnd");
      Vector arguments = new Vector();
