...
--                           Same business key may appear many times; drain picks latest.
--   * sfdc_order_line       + subinventory, + stock_status, + sfdc_line_id
--   * inv_transaction       + sfdc_line_id
--   * POOL sentinel REMOVED — reservations now land on the real
...
--   * load_orders           DELETE-then-INSERT (no ON CONFLICT). Latest-per-order
--                           selected via DISTINCT ON. Older inbox rows for the
--                           same sfdc_order_id marked status='superseded'.
--   * load_transactions     Latest-per-(tenant, external_txn_id) via DISTINCT ON;
--                           older inbox rows for the same key marked 'superseded'.
--   * load_stocklevel       DELETE existing opening_balance rows for the file's
--                           batch, INSERT fresh. No ON CONFLICT.
--   * fetch_inventory       Simpler — no POOL exclusion path. Each (subinv,
--                           stock_status) row reports its own on_hand and reserved.
--   * recalculate_stock_balance Rebuilds per-row reservations from order_line.subinv.
--                           POOL rows no longer created.
--
-- What stays the same:
--   * Trigger-based stock_balance maintenance uses ON CONFLICT for the running
--     materialized aggregate. That's internal materialization, not duplicate-event
--     handling — the inbox→processed contract is what the "no UPSERTS" rule
--     targets. (Narrow reading of the directive.)
-- ============================================================================
