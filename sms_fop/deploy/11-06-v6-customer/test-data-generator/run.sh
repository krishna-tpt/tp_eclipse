#!/usr/bin/env bash
# Linux/macOS launcher. Reads defaults from local.properties (copy sample-config.properties first).
set -euo pipefail
...
exec java -cp "$JAR:$LIB" com.michelin.inventorytest.InventoryTestDataLoader \
