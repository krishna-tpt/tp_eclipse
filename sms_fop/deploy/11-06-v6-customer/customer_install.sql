...
CREATE TABLE processed.inv_transaction (
    ...
    sfdc_order_id       TEXT,                                  -- SFDC order id (e.g. "0WO9Z000009f6lpWAA") — audit
    sfdc_line_id        TEXT,                                  -- SFDC line id — audit
    erp_line_id         TEXT,                                  -- NEW in v6 — Oracle SOURCE_LINE_ID; matches sfdc_order_line.erp_external_id
    erp_header_id       TEXT,                                  -- NEW in v6 — Oracle TRANSACTION_SOURCE_ID; matches sfdc_order.erp_external_id
    payload             JSONB,
...
CREATE INDEX idx_inv_transaction_erp_line_link
    ON processed.inv_transaction (erp_line_id) WHERE erp_line_id IS NOT NULL;
CREATE INDEX idx_inv_transaction_erp_header_link
...
CREATE TABLE processed.sfdc_order (
    ...
    erp_external_id   TEXT,                                    -- NEW in v6 — FOP/Oracle SO header ID (TRANSACTION_SOURCE_ID)
...
CREATE INDEX idx_sfdc_order_erp_external_id
    ON processed.sfdc_order (erp_external_id) WHERE erp_external_id IS NOT NULL;

-- sfdc_order_line gains erp_external_id (FOP line ID) + shipped_qty (partial tracker)
CREATE TABLE processed.sfdc_order_line (
    ...
    sfdc_line_id     TEXT,                                       -- SFDC's own line Id (audit / reconciliation)
    erp_external_id  TEXT,                                       -- NEW in v6 — FOP/Oracle SO line ID (SOURCE_LINE_ID).
                                                                 --   Cascade match key: txn arrives → trigger updates the row
    ...
    shipped_qty      NUMERIC(18, 4) NOT NULL DEFAULT 0,          -- NEW in v6 — sum of |signed_qty| from inv_transaction
                                                                 --   when this line == qty, line auto-closes
    uom_id           BIGINT NOT NULL REFERENCES processed.uom(uom_id),
    uom_code         TEXT NOT NULL,
...
CREATE INDEX idx_sfdc_order_line_erp_external_id
...
-- 2. inv_transaction INSERT → stock_balance.on_hand_qty + line-precise cascade
--
-- v6 cascade rules (precedence):
--   a. NEW.erp_line_id IS NOT NULL → match sfdc_order_line.erp_external_id (line-precise; correct for partial + multi-line)
--   b. else if NEW.sfdc_line_id IS NOT NULL → match sfdc_order_line.sfdc_line_id (when payload already gives SFDC line id directly)
--   c. else if NEW.sfdc_order_id IS NOT NULL → match sfdc_order_line.sfdc_order_id (HEADER fallback — only correct for single-line orders)
--   d. else → no cascade
--
-- The matched line gets:
--   shipped_qty += ABS(NEW.signed_qty)
--   line_state  = 'closed' WHEN shipped_qty + ABS(NEW.signed_qty) >= qty ELSE unchanged
-- Trigger 3 (reservation_apply) reacts to the UPDATE and computes the correct
-- reserved_qty delta — see its body for the (qty - shipped_qty) math.
...
BEGIN
    -- on_hand always moves by signed_qty
    INSERT INTO processed.stock_balance (
        tenant_id, tenant_code, product_id, product_code,
        warehouse_id, warehouse_code, subinventory, stock_status,
        lot_id, lot_code, on_hand_qty, reserved_qty,
    ...
    -- Only shipment-like txns (negative signed_qty) cascade to order lines.
    IF NEW.signed_qty < 0 THEN
        IF NEW.erp_line_id IS NOT NULL THEN
            -- Preferred: line-precise via FOP/Oracle line ID
        ...
        ELSIF NEW.sfdc_line_id IS NOT NULL THEN
            -- Fallback: SFDC native line ID (normalized payloads that pre-set it)
            UPDATE processed.sfdc_order_line
               SET shipped_qty = shipped_qty + v_ship_qty,
                   line_state  = CASE WHEN shipped_qty + v_ship_qty >= qty THEN 'closed'
                                      ELSE line_state END,
        ...
        ELSIF NEW.sfdc_order_id IS NOT NULL THEN
            -- Header-only fallback: correct only for single-line orders.
            -- For multi-line orders this closes every open line — operator should
            -- ensure FOP supplies erp_line_id to avoid this path.
            UPDATE processed.sfdc_order_line
               SET shipped_qty = shipped_qty + v_ship_qty,
                   line_state  = CASE WHEN shipped_qty + v_ship_qty >= qty THEN 'closed'
                                      ELSE line_state END,
...
--    v6: reservation target = qty - shipped_qty when active, 0 when not.
--    Delta on each event = NEW_target - OLD_target. This handles:
--      INSERT new open line             → reserve qty
--      UPDATE shipped_qty (partial)     → release ABS(signed_qty)  [trigger 2 set shipped_qty]
--      UPDATE line_state to closed/cancel→ release remaining (qty - shipped)
--      UPDATE subinv/status change      → release OLD-target on OLD row, reserve NEW-target on NEW row
--      DELETE                           → release remaining (qty - shipped)
...
BEGIN
    ...
    -- Treat subinv/status change as: release on OLD row, reserve on NEW row.
    -- Otherwise: delta-update on the single row.
...
BEGIN
    ...
    -- Delegate each remaining pending row to the per-row procedure.
    FOR v_row IN
        SELECT inbox_id
          FROM staging.txn_inbox
         WHERE status = 'pending'
    ...
    LOOP
        PERFORM staging.promote_one_txn(v_row.inbox_id);
        SELECT status INTO v_status FROM staging.txn_inbox WHERE inbox_id = v_row.inbox_id;
        IF v_status = 'processed' THEN v_ok := v_ok + 1; ELSE v_bad := v_bad + 1; END IF;
    END LOOP;
...
-- ============================================================================
-- DRAIN: load_orders — fallback path that delegates to staging.promote_one_order
-- ============================================================================
...
BEGIN
    ...
    FOR v_row IN
        SELECT inbox_id
         ...
         FOR UPDATE SKIP LOCKED
    LOOP
        PERFORM staging.promote_one_order(v_row.inbox_id);
        SELECT status INTO v_status FROM staging.order_inbox WHERE inbox_id = v_row.inbox_id;
        IF v_status = 'processed' THEN v_ok := v_ok + 1; ELSE v_bad := v_bad + 1; END IF;
    END LOOP;
...
BEGIN
    ...
    IF v_payload ? 'TRANSACTION_TYPE' OR v_payload ? 'TRANSACTION_ID' THEN
        ...
        -- v6: FOP order line ID + header ID. These are the cascade match keys.
        v_erp_line_id    := (v_payload->>'SOURCE_LINE_ID')::text;
        v_erp_header_id  := (v_payload->>'TRANSACTION_SOURCE_ID')::text;
        -- sfdc_*  fields stay NULL when FOP doesn't carry them (typical for Oracle shipments)
...
BEGIN
    ...
    IF v_is_sfdc_native THEN
        ...
        v_order_state   := CASE COALESCE(v_payload->>'status__c', '')
                              ...
                              ELSE 'open'
                           END;
        v_order_erp_ext := v_payload->>'erpExternalId__c';  -- FOP/Oracle SO header ID
    ...
    LOOP
        ...
        IF v_line ? 'Product_Code__c' OR (v_line ? 'Id' AND v_line ? 'WorkOrderId') THEN
            ...
            v_line_erp_ext   := v_line->>'erpExternalId__c';   -- v6: FOP/Oracle SO line ID
